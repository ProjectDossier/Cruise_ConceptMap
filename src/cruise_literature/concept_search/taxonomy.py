...
class TaxonomyRDF(Taxonomy):
    ...
    def assign_children(self, node, text, depth, all_CSO_keywords_list=[]):

        child = Concept(node, text)
        if depth == self.MAX_DEPTH:
            return child
        if text in all_CSO_keywords_list:
            child.children = self.get_children(child, depth, all_CSO_keywords_list)
            return child
        else:
            return child

    def get_children(self, query, depth, all_CSO_keywords_list=[]):
        ...
        output = [
            self.assign_children(
                i["x"], i["z"].n3(self.graph.namespace_manager)[1:-4], depth, all_CSO_keywords_list
            )
            for i in res if query.text in all_CSO_keywords_list
        ]
        return output
        ...
        #     for i in res
        # ]
        # return [child for child in output if child.text in all_CSO_keywords_list]
    #[ {'key': children.text, 'dir': 'left', 'parent': concept.text, 'color': 'bluegrad3'} for children in concept.children if children.text in all_CSO_keywords_list]

    def assign_parents(self, node, text, depth, all_CSO_keywords_list=[]):
        parent = Concept(node, text)
        if depth == self.MAX_DEPTH:
            return parent
        parent.parents = self.get_parents(parent, depth, all_CSO_keywords_list=[])
        return parent
    ...
    def get_parents(self, query, depth, all_CSO_keywords_list=[]):
        depth += 1
	...
	# Code to return children and parents CSO taxonomy Mofeed
    def search(self, query, all_CSO_keywords_list=[], depth=0):
        #print("taxonomy", all_CSO_keywords_list)
        ...
        query.children, query.parents = (
            self.get_children(query, depth=depth, all_CSO_keywords_list=all_CSO_keywords_list),
            self.get_parents(query, depth=depth, all_CSO_keywords_list=all_CSO_keywords_list),
        )
        return query
